...
var headers = require('../../../../dist/headers-f027513e.cjs.prod.js');
    ...
    if (url.isTwUrl(this.paymasterUrl)) {
      ...
      if (this.secretKey) {
        ...
        if (typeof globalThis !== "undefined" && "APP_BUNDLE_ID" in globalThis) {
          headers$1["x-bundle-id"] = globalThis.APP_BUNDLE_ID;
        }
      }
  ...
  async signMessage(message) {
    ...
    try {
      const provider = new ethers.providers.StaticJsonRpcProvider({
        url: evm_wallets_abstract_dist_thirdwebDevWalletsEvmWalletsAbstract.chainIdToThirdwebRpc(chainId, this.config.clientId),
        headers: headers$1
      }, chainId);
...
class BaseAccountAPI {
  ...
  async createUnsignedUserOp(httpRpcClient, info, options) {
    ...
    if (useErc20Paymaster) {
      ...
      partialOp.callGasLimit = ethers.BigNumber.from(estimates.callGasLimit);
      partialOp.verificationGasLimit = ethers.BigNumber.from(estimates.verificationGasLimit);
      partialOp.preVerificationGas = ethers.BigNumber.from(estimates.preVerificationGas);
    } else if (gasless) {
